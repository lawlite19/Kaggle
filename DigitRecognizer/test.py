...
def neuralNetwork(input_layer_size,hidden_layer_size,out_put_layer):
    #before = time.time()
    #data_img = np.loadtxt(r"data/train.csv",delimiter=',',skiprows=1,dtype=np.float64)
    #print time.time()-before
    
    now = time.time()
    data_img = pd.read_csv(r"data/train.csv")
    print time.time()-now
    #data_img = data_img.as_matrix()
    #data_img = loadmat_data(r"data/train.csv")
    X = data_img[:,0:-1]
    y = data_img[:,-1]
